...
TEST(HeaderRewrite, InternalModuleWithMacroReachableInternalsExported) {
  ...
  // public macro (one reason a module counts as public), so it must be re-exported like a normal
  // module — the consumer reaches it via the macro through the umbrella.
  std::set<std::string> public_modules = {"test_lib.foo"};
  auto r = rewrite_header(data_path("test_lib/bar.h"), "test_lib.bar", RewriteOptions{.combined_macros = false, .include_to_module = include_map, .reachable_fqns = {}, .extern_cxx = true, .extra_args = {"-I", gDataDir}, .no_internal_filter = false, .import_std = false, .public_modules = public_modules});
